for patient = 1:9
    ...
    for sensor = 1:sensors
        if(datac(f,sensor)>mean_value_c)
            mask_c(f,sensor)=1;
        end
    end
...
figure()
subplot(2,1,1)
boxplot(c_fea,{'step interval(fast)','step depth','step counts','sample entropy','correlation','step interval(corr)','locomotion','dominant f','freeze Index','all energy', 'smoothness','wavelet mean','porion'});
