...
clear;
    ...
    %% trining data, walking period and stop period
    ...
    pos{8} = [650,1600;2000,4300;6200,6400;];
    pos{9} = [400,1200;1500,2500;49000,50000];
   ...
   [dthreshold,thresholds,mask,parameters,dtth,TG]= self_adaptive(data,pos{patient},stepSize,windowsize,sensors,Features,Thresholds,Thresholds_parameters,classifier,patient);
    ...
    if(patient<8)
    [result2,labels2] = x_fi(data2,windowsize,stepSize,sensors,Features,Thresholds,thres_para);    
    result = [result2];
    labels = [labels2];
    else
    pole = 1500*2;
    [result2,labels2] = x_fi(data,windowsize,stepSize,sensors,Features,Thresholds,thres_para);    
    result = [result2(pole:2100*2)];
    labels = [ labels2(pole:2100*2)];
    ...
    [recall(patient),correctness(patient),specifity(patient)] = calculate_TF(result,labels,1)
