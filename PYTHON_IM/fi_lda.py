...
def lda_analysis(features,Features,filter_ori,metadata,mask):
    '''
    @ features : features extracted from windows
    @ Features : lda features
    @ filter_ori : mask generated by feature threshold
    @ mask : result of feature selection
    This function do the lda and one dimensional decision tree using new metric or gini index
    '''
    # stack all features into one big array
    ...
    # refill W, the features in channels that are not highly correlated with FoG is padded using 0
    cl=0
    ...
    # calculate the mean of FoG and nonFoG windows, get the number of windows of true FoG and true non-FoG
    ...
    ## Saving some data, not useful for training, only for GUI displaying
